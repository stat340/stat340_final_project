...
+ Sampling: for j in 1,...30, $n_{j}$:
\begin{equation}
\begin{gathered}
\mathbf{Y}_{j}|\mathbf{p}_{j}, \mathbf{n}_{j} \stackrel{i.i.d.}{\sim} \binom{\mathbf{n}_{j}}{\mathbf{p}_{j}}\\
\mathbf{p}_{j} = \mathbf{\beta_{0}} +  \mathbf{\beta_{1}} * Type + \mathbf{\beta_{2}} * Zscore\hspace{0.1cm}Enrollment + \mathbf{\beta_{3}} * Zscore\hspace{0.1cm}TestRate
\end{gathered}
\end{equation}
...
logit(\mu_{j}) = \beta_{0} +\beta_{1} * type_{j} + beta3 * test_rate_{j}\\
\phi_{j} = exp(logeta_{j})\\
logeta_{j} \sim logis(logn, 1)
\end{gathered}
\end{equation}
