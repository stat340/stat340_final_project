...
+ Sampling: for i in 1...39 and k in 1, 2, 3, 4:
\begin{equation}
\begin{gathered}
...
logit({\theta}_{i}) = \beta_0+\beta_1\times rice_i^{*}+\beta_2 \times temp_i^{*}
\end{gathered}
\end{equation}
...
logit(p_{j}) = \beta_{0} + \beta_{1} * Type_j + \beta_{2} \times Enrollment_j^{*} + \beta_{3} \times TestRate_j^{*}
\end{gathered}
\end{equation}
...
For both the Hierarchical Model and the Regression Model, the trace plots and ACF plots have all shown that our draws are well-mixed and our parameters have converged to their posterior region. The overlaid density plots have shown that the draws from all three chains all converged to the same distribution with similar density, for both of the models. The posterior predictive also suggests a good adequacy of the models, as the observed mean lies in the center of the distribution of the simulated data mean. 

However, for the Regression Model, the residual plot indicates that there is potential violation of linearity and constant variance, raising some red flags. Considering the limited data and complexity of the model, we chose to proceed with our analysis while acknowledging this issue. 

For Hierarchical Model, given our prior and data, there is a 95% chance that the ratio of Fall 2021's positivity rate over Fall 2020's  positivity rate is within the range from 0.218 to 0.652, indicating that the Carleton is doing better in terms of controlling COVID in Fall 2021 comparing to Fall 2020. Additionally, for Fall 2020, the positivity rate is significantly higher than the rest of the terms as shown in Figure 3. 
...
For Regression Model, given our prior and data, only the slope term ($\beta_2$) between Carleton COVID cases and Rice County's temperature is insignificant. For one standard deviation increase of standardized Rice County Cases, the expected percentage increase in the odds of a Carleton COVID test is 103% and there is a 95% probability that the increase of odds will be within range from 80% to 129%, given the prior and data. For one standard deviation increase of standardized Rice County Temperature, the expected percentage change in the odds of a Carleton COVID test is -2% and there is a 95% probability that the percentage change of odds will be from 21% lower to 20% higher, given the prior and data. 
...
Using Model 2, we know that if the test rate is at their average level among these institutions, the baseline probability of one test being positive for University is 0.0451 ($\beta_{0}$), this baseline level could reasonably range from 0.0301 to 0.0700 with a 95% probability. For LAC we will see an average of 66.32% decrease in the odds of one COVID testing being positive ($\beta_{1})$. However, the odds of one COVID test being positive in a LAC could be from 23.37% to 84.00% lower than the odds of one COVID testing being positive in a University with a 95% probability. Holding the institution type still, with every increase of 1 in the test rate variable, there will be a 2.89% decrease in the odds of one COVID testing being positive ($\beta_{3}$). This decrease could range from 1.21% to 4.60%, with a 95% possibility. 
...
The first two models provide valuable insights into how Carleton has handled COVID from 2020 to 2021. From the hierarchical model, the significant spike in Fall Term 2020 denoting the probability of a single COVID test being positive stood out from the other terms. From the regression model, temperature is shown to be an insignificant factor that affects Carleton's COVID test rate. However, the standardized Rice County COVID cases are shown to be significant in terms of predicting Carleton's COVID cases. Specifically, a one standard deviation increase in the standardized Rice County COVID cases variable is correlated with a 103% increase of the odds of a Carleton COVID test being positive. This is congruent to the fact that as Carleton is located in Rice County, a lot of transmission came from the residents and workers in Rice County. 
Hence, the COVID cases in Carleton campus community should be very positively correlated with Rice Country's situation. The residual plot from the regression model indicates that further modification of the model is needed, as the residuals are not randomly distributed against the fitted value. In the future, the standardized temperature in Rice County could be potentially dropped and multiple other terms like the humidity level, number of activities every week in Carleton, could be added and tested.  


The latter two models regarding COVID cases on a wider range of institutions reveal more information about school to school difference in containing COVID. In specific, we could see that populated universities with lower test rates usually see higher positivity rates than smaller liberal art colleges that conduct more frequent tests over their populations. The interesting finding is that although from posterior predictive distribution, Carleton College has a relatively low COVID positivity rate among 30 institutions, this performance is in fact below average among the 10 liberal arts colleges in the sample, meaning that Carleton still has much to work on when comparing with its peers. 


Our models have certain limitations that should be well acknowledged. For example, the assumptions for linear regression models are not well satisfied, especially when dealing with limited data and extreme outliers. Despite satisfying effective sample sizes, the amount of available data we have access to is still limited due to various reporting standards, thus our conclusion is not strongly generalizable when applying to more institutions. Some variables we selected, for example, the number of enrollment and test rates, share some correlation with each other, raising issues of collinearity. We could also choose more informative priors when setting up the models, since the simple linear regression models are not robust to the choice of hyperpriors. 


In summary, our project provides a positive answer to the research question about whether Carleton handled COVID well, with the levels of positivity rates decreasing over the terms. 
We also identified the situation in the surrounding area as a significant predictor for COVID cases within Carleton. 
